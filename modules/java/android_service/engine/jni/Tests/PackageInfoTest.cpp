...
{
    PackageInfo info(PackOpenCVersion(2,3), PLATFORM_UNKNOWN, ARCH_ARMv7 | FEATURES_HAS_VFPv3);
    string name = info.GetFullName();
...
{
    PackageInfo info(PackOpenCVersion(2,3), PLATFORM_UNKNOWN, ARCH_ARMv7 | FEATURES_HAS_VFPv3 | FEATURES_HAS_NEON);
    string name = info.GetFullName();
...
{
    PackageInfo info("org.opencv.lib_v24_tegra3");
    EXPECT_EQ(PackOpenCVersion(2,4), info.GetVersion());
...
{
    PackageInfo info(PackOpenCVersion(2,3), PLATFORM_TEGRA2, 0);
    string name = info.GetFullName();
...
{
    PackageInfo info1(PackOpenCVersion(2,3), PLATFORM_TEGRA2, 0);
    PackageInfo info2("org.opencv.lib_v23_tegra2");
    EXPECT_TRUE(info1 == info2);
}

TEST(PackageInfo, Comparator2)
{
    PackageInfo info1(PackOpenCVersion(2,4), PLATFORM_UNKNOWN, ARCH_ARMv7 | FEATURES_HAS_NEON | FEATURES_HAS_VFPv3);
    PackageInfo info2("org.opencv.lib_v24_armv7_vfpv3_neon");
    EXPECT_TRUE(info1 == info2);
}
