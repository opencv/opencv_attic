...
}\cvcodePy{ApproxChains(src\_seq,storage,method=CV\_CHAIN\_APPROX\_SIMPLE,parameter=0,minimal\_perimiter=0,recursive=0)-> chains}
...
}\cvcodePy{ArcLength(curve,slice=CV\_WHOLE\_SEQ,is\_closed=-1)-> double}
...
}\cvcodePy{BoundingRect(points,update=0)-> CvRect}
...
}\cvcodePy{CalcPGH(contour,hist)-> None}
...
}\cvcodePy{CalcEMD2(signature1, signature2, distance\_type, distance\_func = None, cost\_matrix=None, flow=None, lower\_bound=None, userdata = None) -> float}
...
}\cvcodePy{CheckContourConvexity(contour)-> int}
...
}\cvcodePy{ContourAres(contour,slice=CV\_WHOLE\_SEQ)-> double}
...
}\cvcodePy{ContourFromContourTree(tree,storage,criteria)-> contour}
...
}\cvcodePy{ConvexHull2(points,storage,orientaton=CV\_CLOCKWISE,return\_points=0)-> convex\_hull}
...
}\cvcodePy{ConvexityDefects(contour,convexhull,storage)-> convexity\_defects}
...
}\cvcodePy{CreateCountourTree(contour,storage,threshold)-> contour\_tree}
...
}\cvcodePy{FindContours(image, storage, mode=CV\_RETR\_LIST, method=CV\_CHAIN\_APPROX\_SIMPLE, offset=(0,0)) -> cvseq}
...
}\cvcodePy{FitEllipse2(points)-> Box2D}
...
}\cvcodePy{FitLine(points, dist\_type, param, reps, aeps) -> line}
...
}\cvcodePy{GetCentralMoment(cvmoments, x\_order, y\_order) -> double}
...
}\cvcodePy{GetNormalizedCentralMoment(cvmoments, x\_order, y\_order) -> double}
...
}\cvcodePy{GetSpatialMoment(cvmoments, x\_order, y\_order) -> double}
...
}\cvcodePy{MatchContourTrees(tree1,tree2,method,threshold)-> double}
...
}\cvcodePy{MatchShapes(object1,object2,method,parameter=0)-> None}
...
}\cvcodePy{MinAreaRect2(points,storage)-> CvBox2D}
...
}\cvcodePy{MinEnclosingCircle(points)-> int,center,radius}
...
}\cvcodePy{Moments(arr) -> cvmoments}
...
}\cvcodePy{PointPolygonTest(contour,pt,measure\_dist)-> double}
...
See also: \cvCppCross{getAffineTransform}, \cvCppCross{getPerspectiveTransform}, \cvCppCross{estimateRigidMotion},
\cvCppCross{warpPerspective}
