...
\par double scale,
\par CvMat* mapMatrix );
}\cvcodePy{2DRotationMatrix(center,angle,scale,mapMatrix)-> None}
...
\par CvMat* mapMatrix );
}\cvcodePy{GetAffineTransform(src,dst,mapMatrix)-> None}
...
\par const CvPoint2D32f* dst,
\par CvMat* mapMatrix );
}\cvcodePy{GetPerspectiveTransform(src,dst,mapMatrix)-> None}
...
}\cvcodePy{GetQuadrangleSubPix(src,dst,mapMatrix)-> None}
...
\cvcodeC{void cvGetRectSubPix(
\par const CvArr* src,
\par CvArr* dst,
\par CvPoint2D32f center );
}\cvcodePy{GetRectSubPix(src,dst,center)-> None}
...
\cvcodeC{void cvInitUndistortMap( \par const CvMat* intrinsic\_matrix,\par const CvMat* distortion\_coeffs,\par CvArr* mapx,\par CvArr* mapy );
}\cvcodePy{InitUndistortMap(camera\_matrix,distortion\_coeffs,mapx,mapy)-> None}
...
}\cvcodePy{InitUndistortRectifyMap(camera\_matrix,dist\_coeffs,R,new\_camera\_matrix,mapx,mapy)-> None}
...
\cvcodePy{LogPolar(src,dst,center,M,flags=CV\_INNER\_LINEAR+CV\_WARP\_FILL\_OUTLIERS)-> None}
...
\cvcodePy{Remap(src,dst,mapx,mapy,flags=CV\_INNER\_LINEAR+CV\_WARP\_FILL\_OUTLIERS,fillval=cvScalarAll0))-> None}
...
\cvcodePy{Resize(src,dst,interpolation=CV\_INTER\_LINEAR)-> None}
...
\cvcodeC{void cvUndistort2( \par const CvArr* src,\par CvArr* dst,\par const CvMat* intrinsic\_matrix,\par const CvMat* distortion\_coeffs );
}
\cvcodePy{Undistort2(src,dst,intrinsic\_matrix,distortion\_coeffs)-> None}
...
\cvcodePy{WarpAffline(src,dst,mapMatrix,flags=CV\_INTER\_LINEAR+CV\_WARP\_FILL\_OUTLIERS,fillval=cvScalarAll(0))-> None}
...
\par int flags=CV\_INTER\_LINEAR+CV\_WARP\_FILL\_OUTLIERS,
\par CvScalar fillval=cvScalarAll(0) );
}\cvcodePy{WarpPerspective(src,dst,mapMatrix,flags=CV\_I
