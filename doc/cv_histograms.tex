...
}\cvcodePy{CalcBackProject(image,back\_project,hist)-> None}
...
}\cvcodePy{CalcBackProjectPatch(images,dst,patch\_size,hist,method,factor)-> None}
...
}\cvcodePy{CalcHist(image,hist,ccumulate=0,mask=NULL)-> None}
...
}\cvcodePy{CalcProbDensity(hist1,hist2,dst\_hst,scale=255)-> None}
...
}\cvcodePy{CompareHist(hist1,hist2,method)->float}
...
}\cvcodePy{CreateHist(dims, type, ranges, uniform = 1) -> hist}
...
}\cvcodePy{GetMinMaxHistValue(hist)-> min\_val,max\_val,min\_loc,max\_loc}
...
}\cvcodePy{NormalizeHist(hist,factor)-> None}
...
}\cvcodePy{ThreshHist(hist,threshold)-> None}
