...
\cross{cvMemStorageAlloc} function or implicitly by higher-level functions,
such as \cross{cvSeqPush}, \cross{cvGraphAddEdge}, etc., \texttt{always}
...
from the parent, see \cross{cvCreateChildMemStorage}) and added to the end of
...
\cross{cvSaveMemStoragePos}, restored via \cross{cvRestoreMemStoragePos},
or reset via \cross{cvClearStorage}.
...
The structure described above stores the position of the stack top that can be saved via \cross{cvSaveMemStoragePos} and restored via \cross{cvRestoreMemStoragePos}.
...
\texttt{v\_next()} and
\texttt{v\_prev()}.
Some OpenCV functions (for example \cross{cvFindContours}) can return multiple CvSeq objects, connected by these relations.
...
first, \cross{cvSliceLength} is called to determine the length of the slice,
then, \texttt{startIndex} of the slice is normalized similarly to the
argument of \cross{cvGetSeqElem} (i.e., negative indices are allowed). The
actual slice to process starts at the normalized \texttt{startIndex}
and lasts \cross{cvSliceLength} elements (again, assuming the sequence is
...
using the \cross{cvSeqSlice} function, or stored into a continuous
buffer with \cross{CvtSeqToArray} (optionally, followed by
\cross{cvMakeSeqHeaderForArray}).
...
\cvcodeC{void cvClearMemStorage( CvMemStorage* storage );}
...
\cvcodeC{void cvClearSeq( CvSeq* seq );}
\cvcodePy{ClearSeq(seq)-> None}
...
\cvcodeC{void cvClearSet( CvSet* setHeader );}
...
\cvcodeC{CvGraph* cvCloneGraph( \par const CvGraph* graph,\par CvMemStorage* storage );}
...
\cvcodeC{CvSeq* cvCloneSeq( \par const CvSeq* seq,\par CvMemStorage* storage=NULL );}
\cvcodePy{CloneSeq(seq,storage)-> None}
...
\cvcodeC{CvMemStorage* cvCreateChildMemStorage(CvMemStorage* parent);}
...
\cvcodeC{CvGraph* cvCreateGraph( \par int graph\_flags,\par int header\_size,\par int vtx\_size,\par int edge\_size,\par CvMemStorage* storage );}
...
\cvarg{mask}{Event mask indicating which events are of interest to the user (where \cross{cvNextGraphItem} function returns control to the user) It can be \texttt{CV\_GRAPH\_ALL\_ITEMS} (all events are of interest) or a combination of the following flags:
...
The function creates a structure for depth-first graph traversal/search. The initialized structure is used in the \cross{cvNextGraphItem} function - the incremental traversal procedure.
...
\cvcodeC{CvMemStorage* cvCreateMemStorage( int blockSize=0 );}
\cvcodePy{CreateMemStorage(blockSize) -> memstorage}
...
\cvcodeC{CvSeq* cvCreateSeq( \par int seqFlags,\par int headerSize,\par int elemSize,\par CvMemStorage* storage);}
...
default value (that may be reassigned using the \cross{cvSetSeqBlockSize}
...
\cvcodeC{CvSet* cvCreateSet( \par int set\_flags,\par int header\_size,\par int elem\_size,\par CvMemStorage* storage );}
...
The function creates an empty set with a specified header size and element size, and returns the pointer to the set. This function is just a thin layer on top of \cross{cvCreateSeq}.

\cvCPyFunc{CvtSeqToArray}
Copies a sequence to one continuous block of memory.

\cvcodeC{void* cvCvtSeqToArray( \par const CvSeq* seq,\par void* elements,\par CvSlice slice=CV\_WHOLE\_SEQ );}
...
\cvcodeC{CvSeq* cvEndWriteSeq( CvSeqWriter* writer );}
...
modified safely. See \cross{cvcvStartWriteSeq} and \cross{cvcvStartAppendToSeq}
...
\cvcodeC{void cvFlushSeqWriter( CvSeqWriter* writer );}
...
\cross{cvSeqPush} instead.

\cvCPyFunc{GetGraphVtx}
Finds a graph vertex by using its index.

\cvcodeC{CvGraphVtx* cvGetGraphVtx( \par CvGraph* graph,\par int vtx\_idx );}
...
\cvcodeC{char* cvGetSeqElem( const CvSeq* seq, int index );}
...
indices always cause the \cross{cvGetSeqElem} call. The function has O(1)
...
\cvcodeC{int cvGetSeqReaderPos( CvSeqReader* reader );}
...
\cvcodeC{CvSetElem* cvGetSetElem( \par const CvSet* setHeader,\par int index );}
...
The function finds a set element by its index. The function returns the pointer to it or 0 if the index is invalid or the corresponding node is free. The function supports negative indices as it uses \cross{cvGetSeqElem} to locate the node.

\cvCPyFunc{GraphAddEdge}
Adds an edge to a graph.

\cvcodeC{int cvGraphAddEdge( \par CvGraph* graph,\par int start\_idx,\par int end\_idx,\par const CvGraphEdge* edge=NULL,\par CvGraphEdge** inserted\_edge=NULL );
}
...
\cvcodeC{int cvGraphAddEdgeByPtr( \par CvGraph* graph,\par CvGraphVtx* start\_vtx,\par CvGraphVtx* end\_vtx,\par const CvGraphEdge* edge=NULL,\par CvGraphEdge** inserted\_edge=NULL );}
...
\cvcodeC{void cvGraphRemoveEdge( \par CvGraph* graph,\par int start\_idx,\par int end\_idx );}
...
\cvcodeC{void cvGraphRemoveEdgeByPtr( \par CvGraph* graph,\par CvGraphVtx* start\_vtx,\par CvGraphVtx* end\_vtx );}
...
\cvcodeC{int cvGraphRemoveVtx( \par CvGraph* graph,\par int index );}
...
\cvcodeC{int cvGraphRemoveVtxByPtr( \par CvGraph* graph,\par CvGraphVtx* vtx );}
...
\cvcodeC{int cvGraphVtxIdx( \par CvGraph* graph,\par CvGraphVtx* vtx );}
...
\cvcodeC{void cvInsertNodeIntoTree( \par void* node,\par void* parent,\par void* frame );}
...
\cvcodeC{CvSeq* cvMakeSeqHeaderForArray( \par int seq\_type,\par int header\_size,\par int elem\_size,\par void* elements,\par int total,\par CvSeq* seq,\par CvSeqBlock* block );}
...
\cvcodeC{void* cvMemStorageAlloc( \par CvMemStorage* storage,\par size\_t size );}
...
\cvcodeC{CvString cvMemStorageAllocString(CvMemStorage* storage, const char* ptr, int len=-1);}
...
in the \texttt{mask} in the \cross{cvCreateGraphScanner} call) is met or the
...
\cvcodeC{void cvReleaseMemStorage( CvMemStorage** storage );}
...
\cvcodeC{void cvRestoreMemStoragePos(\par CvMemStorage* storage,\par CvMemStoragePos* pos);}
...
\cvcodeC{void cvSaveMemStoragePos(\par const CvMemStorage* storage,\par CvMemStoragePos* pos);}
...
\cvcodeC{int cvSeqElemIdx( \par const CvSeq* seq,\par const void* element,\par CvSeqBlock** block=NULL );}
...
\cvcodeC{char* cvSeqInsert( \par CvSeq* seq,\par int beforeIndex,\par void* element=NULL );}

\begin{description}
\cvarg{seq}{Sequence}
\cvarg{beforeIndex}{Index before which the element is inserted. Inserting before 0 (the minimal allowed value of the parameter) is equal to \cross{cvSeqPushFront} and inserting before \texttt{seq->total} (the maximal allowed value of the parameter) is equal to \cross{cvSeqPush}.}
...
\cvcodeC{void cvSeqInsertSlice( \par CvSeq* seq,\par int beforeIndex,\par const CvArr* fromArr );}
...
\cvcodeC{void cvSeqInvert( CvSeq* seq );}
\cvcodePy{SeqInvert(seq)-> None}
...
\cvcodeC{void cvSeqPop( \par CvSeq* seq,\par void* element=NULL );}
...
\cvcodeC{void cvSeqPopFront( \par \par CvSeq* seq,\par\par void* element=NULL );}
...
\cvcodeC{void cvSeqPopMulti( \par CvSeq* seq,\par void* elements,\par int count,\par int in\_front=0 );}
...
\cvcodeC{char* cvSeqPush( \par CvSeq* seq,\par void* element=NULL );}
...
The function has O(1) complexity, but there is a faster method for writing large sequences (see \cross{cvStartWriteSeq} and related functions).


\cvCPyFunc{SeqPushFront}
Adds an element to the beginning of a sequence.

\cvcodeC{char* cvSeqPushFront( CvSeq* seq, void* element=NULL );}
...
The function is similar to \cross{cvSeqPush} but it adds the new element to the beginning of the sequence. The function has O(1) complexity.

\cvCPyFunc{SeqPushMulti}
Pushes several elements to either end of a sequence.

\cvcodeC{void cvSeqPushMulti( \par CvSeq* seq,\par void* elements,\par int count,\par int in\_front=0 );}
...
\cvcodeC{void cvSeqRemove( \par CvSeq* seq,\par int index );}
\cvcodePy{SeqRemove(seq,index)-> None}
...
\cvcodeC{void cvSeqRemoveSlice( CvSeq* seq, CvSlice slice );}
\cvcodePy{SeqRemoveSlice(seq,slice)-> None}
...
\cvarg{func}{The comparison function that returns negative, zero or positive value depending on the relationships among the elements (see also \cross{cvSeqSort})}
...
\cvcodeC{CvSeq* cvSeqSlice( \par const CvSeq* seq,\par CvSlice slice,\par CvMemStorage* storage=NULL,\par int copy\_data=0 );}
...
\cvcodeC{void cvSeqSort( CvSeq* seq, CvCmpFunc func, void* userdata=NULL );}
...
\cvcodeC{int cvSetAdd( \par CvSet* setHeader,\par CvSetElem* elem=NULL,\par CvSetElem** inserted\_elem=NULL );}
...
a faster function for allocating set nodes (see \cross{cvSetNew}).

\cvCPyFunc{SetNew}
Adds an element to a set (fast variant).

\cvcodeC{CvSetElem* cvSetNew( CvSet* setHeader );}

\begin{description}
\cvarg{setHeader}{Set}
\end{description}

The function is an inline lightweight variant of \cross{cvSetAdd}. It occupies a new node and returns a pointer to it rather than an index.


\cvCPyFunc{SetRemove}
Removes an element from a set.

\cvcodeC{void cvSetRemove( \par CvSet* setHeader,\par int index );}
...
\cross{cvSetRemoveByPtr} provides a quicker way to remove a set element
if it is located already.

\cvCPyFunc{SetRemoveByPtr}
Removes a set element based on its pointer.

\cvcodeC{void cvSetRemoveByPtr( \par CvSet* setHeader,\par void* elem );}
...
The function is an inline lightweight variant of \cross{cvSetRemove} that requires an element pointer. The function does not check whether the node is occupied or not - the user should take care of that.


\cvCPyFunc{SetSeqBlockSize}
Sets up sequence block size.

\cvcodeC{void cvSetSeqBlockSize( \par CvSeq* seq,\par int deltaElems );}
...
\cvcodeC{void cvSetSeqReaderPos( \par CvSeqReader* reader,\par int index,\par int is\_relative=0 );}
...
\cvcodeC{void cvStartAppendToSeq( \par CvSeq* seq,\par CvSeqWriter* writer );}
...
\cross{cvFlushSeqWriter}, which helps to avoid some of these problems).

\cvCPyFunc{StartReadSeq}
Initializes the process of sequential reading from a sequence.

\cvcodeC{void cvStartReadSeq( \par const CvSeq* seq,\par CvSeqReader* reader,\par int reverse=0 );}
...
\cross{cvCreateSeq} and \cross{cvStartAppendToSeq}. The pointer to the
created sequence is stored at
\texttt{writer->seq}
and is also returned by the
\cross{cvEndWriteSeq} function that should be called at the end.
