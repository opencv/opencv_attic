...
\cvcodeC{void cvConvertImage( const CvArr* src, CvArr* dst, int flags=0 );}
...
\cvcodePy{CreateTrackbar(trackbarName, windowName, value, count, onChange)}
\begin{lstlisting}
...
\cvcodeC{void cvDestroyAllWindows(void);}
\cvcodePy{DestroyAllWindows()-> None}
...
\cvcodeC{void cvDestroyWindow( const char* name );}
\cvcodePy{DestroyWindow(name)-> None}
...
\cvcodeC{int cvGetTrackbarPos( \par const char* trackbarName, \par const char* windowName );}
\cvcodePy{GetTrackbarPos(trackbarName,windowName)-> None}
...
\cvcodeC{void* cvGetWindowHandle( const char* name );}
...
\cvcodeC{const char* cvGetWindowName( void* windowHandle );}
...
\cvcodeC{int cvInitSystem( int argc, char** argv );}
...
\cvcodeC{void cvMoveWindow( const char* name, int x, int y );}
\cvcodePy{MoveWindow(name,x,y)-> None}
...
\cvcodeC{int cvNamedWindow( const char* name, int flags );}
\cvcodePy{NamedWindow(name,flags=CV\_WINDOW\_AUTOSIZE)-> None}
...
\cvcodeC{void cvResizeWindow( const char* name, int width, int height );}
\cvcodePy{ResizeWindow(name,width,height)-> None}
...
\cvcodeC{void cvSetMouseCallback( const char* windowName, CvMouseCallback onMouse, void* param=NULL );}
\cvcodePy{SetMouseCallback(windowName, onMouse, param) -> None}
...
}\cvcodePy{Foo(event,x,y,flags,param)-> None}
...
\cvcodeC{void cvSetTrackbarPos( \par const char* trackbarName, \par const char* windowName, \par int pos );}
\cvcodePy{SetTrackbarPos(trackbarName,windowName)-> None}
...
\cvcodeC{void cvShowImage( const char* name, const CvArr* image );}
\cvcodePy{ShowImage(name,image)-> None}
...
\cvcodeC{int cvWaitKey( int delay=0 );}
\cvcodePy{WaitKey(delay=0)-> int}
...
\cvcodePy{LoadImage(filename, iscolor=CV\_LOAD\_IMAGE\_COLOR)}
...
\cvcodeC{int cvSaveImage( const char* filename, const CvArr* image );}
\cvcodePy{SaveImage(filename,image)-> None}
...
\cvcodeC{typedef struct CvCapture CvCapture;}
...
\cvcodeC{CvCapture* cvCaptureFromCAM( int index );}
\cvcodePy{CaptureFromCAM(index) -> CvCapture}
...
\cvcodeC{CvCapture* cvCaptureFromFile( const char* filename );}
\cvcodePy{CaptureFromFile(filename) -> CvCapture}
...
\cvcodeC{double cvGetCaptureProperty( CvCapture* capture, int property\_id );}
\cvcodePy{GetCaptureProperty(capture, property\_id)->double}
...
\cvcodeC{int cvGrabFrame( CvCapture* capture );}
\cvcodePy{GrabFrame(capture) -> int}
...
\cvcodeC{IplImage* cvQueryFrame( CvCapture* capture );}
\cvcodePy{QueryFrame(capture) -> iplimage}
...
\cvcodeC{void cvReleaseCapture( CvCapture** capture );}
...
\cvcodeC{IplImage* cvRetrieveFrame( CvCapture* capture );}
\cvcodePy{RetrieveFrame(capture) -> iplimage}
...
\cvcodeC{int cvSetCaptureProperty( \par CvCapture* capture, \par int property\_id, \par double value );}
\cvcodePy{SetCaptureProperty(capture, property\_id,double)}
...
}\cvcodePy{CreateVideoWriter(filename, fourcc, fps, frame\_size, is\_color) -> CvVideoWriter}
...
\cvcodeC{void cvReleaseVideoWriter( CvVideoWriter** writer );}
...
\cvcodeC{int cvWriteFrame( CvVideoWriter* writer, const IplImage* image );}
\cvcodePy{WriteFrame(writer, image)->int}
