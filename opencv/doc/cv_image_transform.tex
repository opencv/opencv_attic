...
}\cvcodePy{AdaptiveThreshold(src,dst,maxValue, adaptive\_method=CV\_ADAPTIVE\_THRESH\_MEAN\_C, thresholdType=CV\_THRESH\_BINARY,blockSize=3,param1=5)-> None}
...
\par CvArr* dst,
\par int code );
}\cvcodePy{CvtColor(src,dst,code)-> None}
...
}\cvcodePy{DistTransform(src,dst,distance\_type=CV\_DIST\_L2,mask\_size=3,mask={NULL,0},labels=NULL)-> None}
...
}\cvcodePy{FloodFill(image,seed\_point,new\_val,lo\_diff=cvScalarAll(0),up\_diff=cvScalarAll(0),flags=4,mask=NULL)-> comp}
...
}\cvcodePy{Inpaint(src,mask,dst,inpaintRadius,flags) -> None}
...
}\cvcodePy{Integral(image,sum,sqsum=NULL,tiltedSum=NULL)-> None}
...
}\cvcodePy{PyrMeanShiftFiltering(src,dst,sp,sr,max\_level=1,
...
}\cvcodePy{PyrSegmentation(src,dst,storage,level,threshold1,threshold2)-> comp}
...
}\cvcodePy{Threshold(src,dst,threshld,maxValue,thresholdType)-> None}
