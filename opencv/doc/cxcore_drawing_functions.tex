...
with \texttt{CV\_RGB} \cvC{macro or the \cross{cvScalar} function}
...
\fi
\ifC
, so if you form a color using \cross{cvScalar}, it should look like:
...
\cvcodeC{void cvCircle( \par CvArr* img,\par CvPoint center,\par int radius,\par CvScalar color,\par int thickness=1,\par int lineType=8,\par int shift=0 );}
\cvcodePy{Circle(img,center,radius,color,thickness=1,lineType=8,shift=0)-> None}
...
\cvcodeC{int cvClipLine( \par CvSize imgSize,\par CvPoint* pt1,\par CvPoint* pt2 );}
\cvcodePy{ClipLine(img, pt1, pt2) -> (clipped\_pt1, clipped\_pt2)}
\begin{description}
...
\cvcodePy{DrawContours(img,contour,external\_color,hole\_color,max\_level,thickness=1,lineType=8,offset=(0,0)-> None}
...
\cvcodeC{void cvEllipse( \par CvArr* img,\par CvPoint center,\par CvSize axes,\par double angle,\par double start\_angle,\par double end\_angle,\par CvScalar color,\par int thickness=1,\par int lineType=8,\par int shift=0 );}
\cvcodePy{Ellipse(img,pt1,axes,angle,start\_angle,end\_angle,color,thickness=1,lineType=8,shift=0)-> None}
...
\cvcodeC{void cvEllipseBox( \par CvArr* img, \par CvBox2D box, \par CvScalar color,
                   \par int thickness=1, \par int lineType=8, \par int shift=0 );}
\cvcodePy{EllipseBox(img,box,color,thickness=1,lineType=8,shift=0)-> None}
...
\cvcodePy{FillConvexPoly(img,pn,color,lineType=8,shift=0)-> None}
...
\cvcodePy{FillPoly(img,polys,color,lineType=8,shift=0)-> None}
...
\cvcodePy{GetTextSize(text\_string,font)-> text\_size,baseline}
...
\cvcodeC{
void cvInitFont( \par CvFont* font,\par int font\_face,\par double hscale,\par double vscale,\par double shear=0,\par int thickness=1,\par int lineType=8 );}
\cvcodePy{InitFont(font\_face,hscale,vscale,shear=0,thickness=1,lineType=8)-> font}
...
\cvcodePy{InitLineIterator(image, pt1, pt2, connectivity=8, left\_to\_right=0)}
...
\cvcodePy{Line(img,pt1,pt2,color,thickness=1,lineType=8,shift=0)-> None}
...
\cvcodePy{PolyLine(img,pts,is\_closed,color,thickness=1,lineType=8,shift=0)-> None}
...
\cvcodePy{PutText(img,text,org,font,color)-> None}
...
\cvcodeC{void cvRectangle( \par CvArr* img,\par CvPoint pt1,\par CvPoint pt2,\par CvScalar color,\par int thickness=1,\par int lineType=8,\par int shift=0 );}
\cvcodePy{Rectangle(img,pt1,pt2,color,thickness=1,lineType=8,shift=0)-> None}
...
\cvcodeC{\#define CV\_RGB( r, g, b )  cvScalar( (b), (g), (r) )}
\cvcodePy{CV\_RGB(red,grn,blu)->CvScalar}
