...
}\cvdefPy{CornerEigenValsAndVecs(image,eigenvv,block\_size,aperture\_size=3)-> None}
...
\par int aperture\_size=3,
\par double k=0.04 );
}\cvdefPy{CornerHarris(image,harris\_dst,block\_size,aperture\_size=3,k=0.04)-> None}
...
\par int block\_size,
\par int aperture\_size=3 );
}\cvdefPy{CornerMinEigenVal(image,eigenval,block\_size,aperture\_size=3)-> None}
\begin{description}
\cvarg{image}{Input image}
...
\cvdefPy{GoodFeaturesToTrack(image,eig\_image,temp\_image,quality\_level,min\_distance,mask=NULL,block\_size=3,use\_harris=0,k=0.04)-> corners}
