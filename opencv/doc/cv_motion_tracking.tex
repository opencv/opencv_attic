...
}\cvcodePy{Acc(image,smu,mask=NULL)-> None}
...
}\cvcodePy{CalcGlobalOrientation(orientation,mask,mhi,timestamp,duration)-> None}
...
}\cvcodePy{CalcMotionGradient(mhi,mask,orientation,delta1,delta2,aperture\_size=3)-> None}
...
\]}
\cvarg{aperture\_size}{Aperture size of derivative operators used by the function: CV\_SCHARR, 1, 3, 5 or 7 (see \cvCPyCross{Sobel})}
\end{description}
...
}\cvcodePy{CalcOpticalFlowBM(prev,curr,block\_size,shift\_size,max\_range,use\_previous,velx,vely)-> None}
...
}\cvcodePy{CalcOpticalFlowHS(prev,curr,use\_previous,velx,vely,lambda,criteria)-> None}
...
}\cvcodePy{CalcOpticalFlowLK(prev,curr,win\_size,velx,vely)-> None}
\begin{description}
...
\cvcodeC{
void cvCalcOpticalFlowPyrLK( \par const CvArr* prev,\par const CvArr* curr,\par CvArr* prevPyr,\par CvArr* currPyr,\par const CvPoint2D32f* prevFeatures,\par CvPoint2D32f* currFeatures,\par int count,\par CvSize win\_size,\par int level,\par char* status,\par float* track\_error,\par CvTermCriteria criteria,\par int flags );
}\cvcodePy{
CalcOpticalFlowPyrLK(  prev, curr, prevPyr, currPyr, prevFeatures, CvSize win\_size, int level, criteria, flags, guesses = None) -> (currFeatures, status, track\_error)
}
...
}\cvcodePy{CamShift(prob\_image,window,criteria,box=NULL)-> comp}
...
\cvstruct{CvKalman}\label{CvKalman}
Kalman filter state.
...
}\cvcodePy{MeanShift(prob\_image,window,criteria)-> comp}
...
}\cvcodePy{MulitplyAcc(image1,image2,acc,mask=NULL)-> None}
...
}\cvcodePy{RunningAvg(image,acc,alpha,mask=NULL)-> None}
...
}\cvcodePy{SegmentMotion(mhi,seg\_mask,storage,timestamp,seg\_thresh)-> None}
...
}\cvcodePy{SnakeImage(image,points,alpha,beta,gamma,coeff\_usage,win,criteria,calc\_gradient=1)-> None}
...
}\cvcodePy{SquareAcc(image,sqsum,mask=NULL)-> None}
...
}\cvcodePy{UpdateMotionHistory(silhouette,mhi,timestamp,duration)-> None}
