...
\cvcodePy{CopyMakeBorder(src,dst,offset,bordrtype,value=cvScalarAll(0))-> None}
...
\cvcodeC{IplConvKernel* cvCreateStructuringElementEx(\par int cols,
 \par int rows, \par int anchorX, \par int anchorY, \par int shape, \par int* values=NULL );}
\cvcodePy{CreateStructuringElementEx(cols,rows,anchorX,anchorY,shape,values={NULL,0})-> kernel}
...
\cvcodeC{void cvDilate(\par const CvArr* src,\par CvArr* dst,\par IplConvKernel* element=NULL,\par int iterations=1 );}
\cvcodePy{Dilate(src,dst,element=NULL,iterations=1)-> None}
...
\cvcodeC{void cvErode(\par const CvArr* src,\par  CvArr* dst,\par  IplConvKernel* element=NULL,\par  int iterations=1);}
\cvcodePy{Erode(src,dst,element=NULL,itertions=1)-> None}
...
\cvcodeC{void cvLaplace(\par const CvArr* src,\par  CvArr* dst,\par  int apertureSize=3);}
\cvcodePy{Laplace(src,dst,apertureSize=3)-> None}
\begin{description}
\cvarg{src}{Source image}
\cvarg{dst}{Destination image}
...
\cvcodeC{void cvPyrDown(\par const CvArr* src,\par CvArr* dst,\par int filter=CV\_GAUSSIAN\_5x5 );}
\cvcodePy{PyrDown(src,dst,filter=CV\_GAUSSIAN\_5X5)-> None}
...
\cvcodeC{void cvReleaseStructuringElement( IplConvKernel** element );}
...
\cvcodeC{void cvSmooth(\par const CvArr* src,
\par CvArr* dst,\par  int smoothtype=CV\_GAUSSIAN,\par  int param1=3,\par  int param2=0,\par  double param3=0, \par double param4=0);}
\cvcodePy{Smooth(src,dst,smoothtype=CV\_GAUSSIAN,param1=3,param2=0,param3=0,param4=0)-> None}
...
\begin{description}
  \cvarg{CV\_BLUR\_NO\_SCALE (simple blur with no scaling)}{summation over a pixel $\texttt{param1}\times \texttt{param2}$ neighborhood. If the neighborhood size varies, one can precompute the integral image with the \cvCPyCross{Integral} function}
...
\cvcodeC{void cvSobel(\par const CvArr* src,\par  CvArr* dst,\par  int xorder,\par  int yorder,\par  int apertureSize=3 );}
\cvcodePy{Sobel(src,dst,xorder,yorder,apertureSize = 3)-> None}
