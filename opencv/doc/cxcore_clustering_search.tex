...
\cvcodeC{void cvKMeans2( \par const CvArr* samples,\par int cluster\_count,\par CvArr* labels,\par CvTermCriteria termcrit);}
\cvcodePy{KMeans2(samples,cluster\_count,labels,termcrit)-> None}
...
\newline (\texttt{flags}=\texttt{KMEANS\_USE\_INITIAL\_LABELS}) flag, and then choose the best (most-compact) clustering.
