...
int main(){
	...
	if(!capture.isOpened()){
		cout<<"Failed to capture video, check the video path (or webcam connection) again, exiting...\n";
		exit(0);
	}

	const char* winTitle = "Camera feed";
	...
	for(;;){
		...
		cv::ocl::calcOpticalFlowLK(OclMat(frame0gray), OclMat(frame1gray), velXgpu, velYgpu, winSize);
