...
void BruteForceMatcherTest::run( int )
{
    ...
    for( int i=0;i<descriptorsNumber;i++ )
    {
        ...
        bool isEquiv = fabs( specMatches[i].distance - genericMatches[i].distance ) < epsilon &&
                       specMatches[i].queryIndex == genericMatches[i].queryIndex &&
                       specMatches[i].trainIndex == genericMatches[i].trainIndex;
        if( !isEquiv || specMatches[i].trainIndex != permutation.at<int>( 0, i ) )
        {
