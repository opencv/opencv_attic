...
\cvarg{ fourcc}{4-character code of codec used to compress the frames. For example,\newline \texttt{CV\_FOURCC('P','I','M','1')} is MPEG-1 codec, \texttt{CV\_FOURCC('M','J','P','G')} is motion-jpeg codec etc. Under Win32 it is possible to pass -1 in order to choose compression method and additional compression parameters from dialog. Under Win32 if we pass 0 while using an avi filename it will create a video writer that creates uncompressed avi file.}
