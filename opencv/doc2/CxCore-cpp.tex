...
\begin{lstlisting}
int kmeans( const Mat& samples, int clusterCount,
            Mat& labels, Mat& centers,
            TermCriteria termcrit, int attempts=1,
...
\cvarg{centers}{The input/output matrix of the cluster centers, one row per each cluster center}
...
splitting a set into one or more equivalancy classes, described in \href{http://en.wikipedia.org/wiki/Disjoint-set_data_structure}. The function
