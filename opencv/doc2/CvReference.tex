...
\cvarg{ aperture\_size}{Aperture parameter for Sobel operator (see \cross{Sobel})}. format. In the case of floating-point input format this parameter is the number of the fixed float filter used for differencing
\end{description}
...
\cvarg{ tilted\_sum}{The integral for the image rotated by 45 degrees, $(W+1)\times (H+1)$, the same data type as `sum`}
\end{description}
...
\cvarg{ code}{Color conversion operation that can be specifed using CV\_<src\_color\_space>2<dst\_color\_space> constants (see below)}
\end{description}
...
\begin{description}
\cvarg{CV\_DIST\_C $(3\times 3)$}{a = 1, b = 1}
\cvarg{CV\_DIST\_L1 $(3\times 3)$}{a = 1, b = 2}
\cvarg{CV\_DIST\_L2 $(3\times 3)$}{a=0.955, b=1.3693}
\cvarg{CV\_DIST\_L2 $(5\times 5)$}{a=1, b=1.4, c=2.1969}
\end{description}
...
\cvarg{ orientation}{Motion gradient orientation image; contains angles from 0 to ~360°}
