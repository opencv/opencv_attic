...
}{CPP}{Sobel(src,dst,xorder,yorder,aperture_size = 3)-> None}
\begin{description}
...
}{CPP}{Laplace(src,dst,aperture_size=3)-> None}
\begin{description}
...
}{CPP}{Canny(image,edges,threshold1,threshold2,aperture_size=3)-> None}
\begin{description}
...
}{CPP}{PreCornerDetect(image,corners,aperture_size=3)-> None}
\begin{description}
...
}{CPP}{CornerEigenValsAndVecs(image,eigenvv,block_size,aperture_size=3)-> None}
...
}{CPP}{CornerMinEigenVal(image,eigenval,block_size,aperture_size=3)-> None}
\begin{description}
\cvarg{image}{Input image}
...
}{CPP}{CornerHarris(image,harris_dst,block_size,aperture_size=3,k=0.04)-> None}
...
}{CPP}{FindCornerSubPix(image,win,zero_zone,criteria)-> corners}
...
}{CPP}{GoodFeaturesToTrack(image,eig_image,temp_image,quality_level,min_distance,mask=NULL,block_size=3,use_harris=0,k=0.04)-> corners}
...
}{CPP}{GetStarKeypoints(image,storage,params)-> CvSeqOfStarKeypoint*}
...
}{CPP}{GetQuadrangleSubPix(src,dst,map_matrix)-> None}
...
}{CPP}{Resize(src,dst,interpolation=CV_INTER_LINEAR)-> None}
...
}{CPP}{WarpAffline(src,dst,map_matrix,flags=CV_INTER_LINEAR+CV_WARP_FILL_OUTLIERS,fillval=cvScalarAll(0))-> None}
...
CvMat* map\_matrix );
}{CPP}{GetAffineTransform(src,dst,map_matrix)-> None}
...
CvMat* map\_matrix );
}{CPP}{2DRotationMatrix(center,angle,scale,map_matrix)-> None}
...
}{CPP}{WarpPerspective(src,dst,map_matrix,flags=CV_I
NNER_LINEAR+CV_WARP_FILL_OUTLIERS,fillval=cvScalarAll(0
))-> None}
...
CvMat* map\_matrix );
}{CPP}{GetPerspectiveTransform(src,dst,map_matrix)-> None}
...
}{CPP}{Remap(src,dst,mapx,mapy,flags=CV_INNER_LINEAR+CV_WARP_FILL_OUTLIERS,fillval=cvScalarAll0))-> None}
...
}{CPP}{LogPolar(src,dst,center,M,flags=CV_INNER_LINEAR+CV_WARP_FILL_OUTLIERS)-> None}
...
int* values=NULL );
}{CPP}{CreateStructuringElementEx(cols,rows,anchor_x,anchor_y,shape,values={NULL,0})-> IplConvKernel*}
...
}{CPP}{Smooth(src,dst,smoothtype=CV_GAUSSIAN,param1=3,param2=0,param3=0,param4=0)-> None}
...
}{CPP}{Integral(image,sum,sqsum=NULL,tilted_sum=NULL)-> None}
...
int threshold\_type );
}{CPP}{Threshold(src,dst,threshld,max_value,threshold_type)-> None}
...
}{CPP}{AdaptiveTheshold(src,dst,max_value, adaptive_method=CV_ADAPTIVE_THRESH_MEAN_C, threshold_type=CV_THRESH_BINARY,block_size=3,param1=5)-> None}
...
}{CPP}{PyrDown(src,dst,filter=CV_GAUSSIAN_5X5)-> None}
...
}{CPP}{PyrUp(src,dst,filter=CV_GAUSSIAN_5X5)-> None}
...
}{CPP}{FloodFill(image,seed_point,new_val,lo_diff=cvScalarAll(0),up_diff=cvScalarAll(0),flags=4,mask=NULL)-> comp}
...
}{CPP}{HoughLines2(image,storage,method,rho,theta,threshold,param1=0,parma2=0)-> None}
...
}{CPP}{HoughCircles(image,circle_storage,method,dp,min_dist,param1=100,param2=100,min_radius=0,max_radius=0)-> None}
...
}{CPP}{DistTransform(src,dst,distance_type=CV_DIST_L2,mask_size=3,mask={NULL,0},labels=NULL)-> None}
...
}{CPP}{GetMinMaxHistValue(hist)-> min_val,max_val,min_loc,max_loc}
...
}{CPP}{CalcBackProject(image,back_project,hist)-> None}
...
}{CPP}{CalcBackProjectPatch(images,dst,patch_sze,hist,method,factor)-> None}
...
}{CPP}{CalcProbDensity(hist1,hist2,dst_hst,scale=255)-> None}
...
}{CPP}{ApproxChains(src_seq,storage,method=CV_CHAIN_APPROX_SIMPLE,parameter=0,minimal_perimiter=0,recursive=0)-> CvSeq*}
...
}{CPP}{ContourAres(contour,slice=CV_WHOLE_SEQ)-> double}
...
}{CPP}{ArcLength(curve,slice-CV_WHOLE_SEQ,is_closed=-1)-> double}
...
}{CPP}{CreateCountourTree(contour,storage,threshold)-> CvContourTree*}
...
}{CPP}{ContourFromContourTree(tree,storage,criteria)-> CvSeq*}
...
}{CPP}{ConvexHull2(points,storage,orientaton=CV_CLOCKWISE,return_points=0)-> CvSeq*}
...
}{CPP}{ConvexityDefects(contour,convexhull,storage)-> CvSeqOfCvConvexityDefect*}
...
}{CPP}{PointPolygonTest(contour,pt,measure_dist)-> double}
...
}{CPP}{Subdiv2DEdgeOrg(edge)-> CvSubdiv2DPoint*}
...
}{CPP}{Subdiv2DEdgeDist(edge)-> CvSubdiv2DPoint*}
...
}{CPP}{CreateSubdivDelaunay2D(rect,storage)-> CvSubdiv2D*}
...
}{CPP}{SubdivDelaunay2DInsert(subdiv,pt)-> CvSubdiv2DPoint*}
...
}{CPP}{FindNearestPoint2D(subdiv,pt)-> CvSubdiv2DPoint*}
...
}{CPP}{CalcMotionGradient(mhi,mask,orientation,delta1,delta2,aperture_size=3)-> None}
...
}{CPP}{SegmentMotion(mhi,seg_mask,storage,timestamp,seg_thresh)-> None}
...
}{CPP}{MeanShift(prob_image,window,criteria)-> comp}
...
}{CPP}{CamShift(prob_image,window,criteria,box=NULL)-> comp}
...
}{CPP}{SnakeImage(image,points,alpha,beta,gamma,coeff_usage,win,criteria,calc_gradient=1)-> None}
...
}{CPP}{CalcOpticalFlowHS(prev,curr,use_previous,velx,vely,lambda,criteria)-> None}
...
}{CPP}{CalcObticalFlowLK(prev,curr,win_size,velx,vely)-> None}
\begin{description}
...
}{CPP}{CalcOpticalFlowBM(prev,curr,block_size,shift_size,max_range,use_previous,velx,vely)-> None}
...
}{CPP}{HaarDetectObjects(image,cascade,storage,scale_factor=1.1,min_neighbors=3,flags=0,min_size=(0,0))-> CvSeqOfCvAvgComp*}
...
}{CPP}{ProjectPoints2(object_points,rotation_vector,translation_vector,intrinsic_matrix,distortion_coeffs, image_points,dpdrot=NULL,dpdt=NULL,dpdf=NULL,dpdc=NULL,dpddist=NULL)-> None}
...
}{CPP}{FindHomography(src_points,dst_points)-> homography}
...
}{CPP}{CalibrateCamera2(object_points,image_points,point_counts,image_size,intrinsic_matrix,distortion_coeffs,rotation_vectors,translation_vectors,flags=0)-> None}
...
}{CPP}{FindExtrinsicCameraParams2(object_points,image_points,intrinsic_matrix,distortion_coeffs,rotation_vector,translation_vector)-> None}
...
}{CPP}{Undistort2(src,dst,intrinsic_matrix,distortion_coeffs)-> None}
...
}{CPP}{InitUndistoryMap(camera_matrix,distortion_coeffs,mapx,mapy)-> None}
...
}{CPP}{DrawChessboardCorners(image,pattern_size,corners,pattern_was_found)-> None}
...
}{CPP}{CreatePOSITObject(points)-> CvPOSITObject*}
...
}{CPP}{POSIT(posit_object,image_points,focal_length,criteria)-> rotation_matrix,translation_vector}
